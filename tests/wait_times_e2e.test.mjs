...
test('e2e: Paddington pivot wait reflects Circle headway (covers #1 direction-sensitive)', () => {
  ...
  // The interchange at Paddington should show waitMins=2 (Circle teardrop
  // pivot, combined shared-platform frequency — see commit e5d4502; the
  // pre-fix Circle-only value was 5). If waitTime were poisoned across
  // directions or routing the wrong "to" through firstHopOnLeg, we'd get a
  // different value.
  const userLegsData = makeUserLegsData('Victoria', [
    {station: 'Paddington', line: 'Circle'},
    {station: 'Ladbroke Grove', line: 'Circle'},
